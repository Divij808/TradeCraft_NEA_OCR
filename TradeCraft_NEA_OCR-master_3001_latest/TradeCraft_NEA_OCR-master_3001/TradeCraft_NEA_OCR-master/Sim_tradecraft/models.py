...
# Resolve the module directory to locate data files.
base_directory = os.path.dirname(os.path.abspath(__file__))

# Find companies.json file in common locations.
candidates = [
...
# Load raw company metadata from JSON.
with open(companies_file, "r") as f:
    raw_companies = json.load(f)

# Normalize into a symbol-indexed dictionary.
COMPANIES = {}
...
def list_companies():
    # Return company metadata as a list for rendering.
    return list(COMPANIES.values())

def company_image(symbol):
    # Find the company logo path for a given symbol.
